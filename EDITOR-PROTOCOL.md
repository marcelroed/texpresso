...
The bundled XeTeX engine also records the source position of every glyph of a TFM font (`txp:tag,line,column[,length]` specials before the glyphs whose position does not follow from the previous one). Both directions use these positions first, and fall back to SyncTeX records and text matching for glyphs without one (OpenType fonts, pictures). With `TXP_SYNC_DEBUG` set, `[synctex forward] glyph: ...` and `[synctex backward] glyph: tag T line L column C` give the glyph that was used; `TXP_SYNC_DEBUG_GLYPHS` lists all the glyphs of the target line forward.
